...
function set_hosts_file() {
  ...
  [ -n "$SELINUX_STATE" ] && [ -x /sbin/restorecon ] && /sbin/restorecon /etc/hosts
...
function set_hostname_fqdn_format() {
  ...
  if test "${dot_appear_times_to_match_fqdn_rule}" -gt 1; then
    ...
    if test ! -z ${user_defined_hostname}; then
      ...
      read -rp 'Input hostname you want, then press Enter ' user_input_hostname
...
eof
  if [[ -f /etc/security/limits.d/90-nproc.conf ]]; then
    mv /etc/security/limits.d/90-nproc.conf /etc/security/limits.d/90-nproc.conf~
  fi
...
eof
  ...
  SELINUX_STATE=$(cat "/selinux/enforce")
  [ -n "$SELINUX_STATE" ] && [ -x /sbin/restorecon ] && /sbin/restorecon -r /usr/local/bin
...
eof
  SELINUX_STATE=$(cat "/selinux/enforce")
  ([ -n "$SELINUX_STATE" ] && [ -x /sbin/restorecon ] && /sbin/restorecon -r /root/.ssh) || chcon --user=system_u --role=object_r --type=net_conf_t --range=s0 /etc/hosts
...
function update_local_time() {
  if ! grep -qa CST-8 /etc/localtime || ! diff /etc/localtime /usr/share/zoneinfo/Asia/Shanghai >/dev/null 2>&1; then
    rm -rf /etc/localtime

...
Some helpful message:
    ...
    For Ubuntu: apt-get -y install python-dev python-pip; pip install psutil
...
class dockerContainerUtils(object):
    ...
    @staticmethod
    def isDockerContainerId(containerId):
        import re
        pattern = re.compile(r'([a-z0-9]+)')
        match = pattern.match(str(containerId).strip('[]\'\"').strip())
        if match:
            return True
        else:
            return False
    ...
    def getContainerId(self):
        ...
        if len(match) != 0 and self.isDockerContainerId(match):
            return match[0]
        elif len(self.getContainerLongIdUsingDockerInspectAllContainerID()) > 1:
            ...
            raise RuntimeError("not supported docker version or a bug otherwise, aborted! ")

    def truncateId(self):
        containerID = self.getContainerId()
        if len(containerID) != 0:
            if type(containerID) is list:
                return str(containerID[0])[:12]
            elif type(containerID) is str:
    ...
    def getContainerNameById(self):
        ...
        if len(containerId) != 0:
            import subprocess
            result = subprocess.Popen("docker inspect -f '{{ .Name }}' %s" % containerId[0], shell=True,
                                      stdout=subprocess.PIPE,
                                      stderr=subprocess.STDOUT)
            # also can get this from /run/runc/<id>/state.json or /run/containerd/<id>/state.json
            if result.stdout.read() is not None:

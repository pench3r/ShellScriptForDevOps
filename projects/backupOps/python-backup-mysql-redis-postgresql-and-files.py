...
"""
Created by PyCharm.
File Name:              LinuxBashShellScriptForOps:python-backup-mysql-redis-postgresql-and-files.py
Version:                0.0.1
Author:                 Liuhongda
...
def backup_mysql():
    ...
    cli = """mysqldump -u {user} -p"{password}" -h {host} -P {port} \
        -f -R -E --triggers --single-transaction -B \
        {database} > {backup_dir}/backup_{database}_{now}.sql 2>/dev/null""".format(**mysql_backup_config)
    call(cli, shell=True)

    sql_name = "{backup_dir}/backup_{database}_{now}.sql".format(**mysql_backup_config)
 ...
 def backup_redis():
    """
    Backup Redis database using `redis-dump`

    redis-dump -u 10.86.77.14:6379 -d 15 > backup_suitecrm_202003171275.sql

    :return:
    """
    cli = """redis-dump -u 10.86.77.14:6379 -d 15 > backup_suitecrm_202003171275.sql"""
    call(cli, shell=True)
...
def backup_postgresql():
    ...
    pg_dump -U postgres -d suitecrm -f backup_suitecrm_202004178264.sql

    :return:
    """
    cli = """pg_dump -U postgres -d suitecrm -f backup_suitecrm_202004178264.sql""".format(**postgresql_backup_config)
    call(cli, shell=True)

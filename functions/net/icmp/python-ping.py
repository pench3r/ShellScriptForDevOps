...
"""
Created by PyCharm.
File Name:              LinuxBashShellScriptForOps:python-ping.py
Version:                0.0.1
Author:                 dgden
...
Description:            
Long Description:       
References:             https://bitbucket.org/delroth/python-ping/
                        https://pypi.org/project/ping/#files
Prerequisites:          []
...
def checksum(source_string):
    ...
    for count in xrange(0, count_to, 2):
        ...
        _sum = _sum + this
        _sum = _sum & 0xffffffff  # Necessary?
...
def receive_one_ping(my_socket, _id, timeout):
    ...
    while True:
        ...
        _type, code, _checksum, packet_id, sequence = struct.unpack(
            "bbHHh", icmpHeader
        )
        if packet_id == _id:
            _bytes = struct.calcsize("d")
            time_sent = struct.unpack("d", received_packet[28:28 + _bytes])[0]
...
def send_one_ping(my_socket, dest_addr, _id, psize):
    """
    ...
    header = struct.pack("bbHHh", ICMP_ECHO_REQUEST, 0, my_checksum, _id, 1)
    _bytes = struct.calcsize("d")
    data = (psize - _bytes) * "Q"

...
def prepare_qqwry():
    ...
    if mswindows:
        map(download_file_to_curdir, [windows_download_url, qqwry_download_url])
        filename_list = map(lambda x: x.split("/")[-1], [windows_download_url, qqwry_download_url])
        map(extract_zip, filename_list)
